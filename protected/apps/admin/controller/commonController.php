...
class commonController extends baseController{
	...
	public function __construct()
	{
		...
		$config['AUTH_LOGIN_NO']=array('index'=> array('login','logout','verify','ajaxReturn','wifiUse','estimateEnter'),'common'=>'*','register'=>'*','dbback'=>array('backup','downloadBackupFile'));//无须验证的方法
	...
	//2016-11-04-btou自己添加的函数
	protected function getmap(){

		$module=CONTROLLER_NAME;//获取当前执行的模块
		$action=ACTION_NAME;//获取当前执行的操作
		$methodInfo = model('method');


		$return['module'] = $methodInfo->find("operate='$module'");
		$return['action'] = $methodInfo->find("rootid=".$return['module']['id']." and operate='$action'");
	...
	//2016-11-06
	protected function istpost($name){


		$data = (isset($_POST[$name]) and (!empty($_POST[$name])))?$_POST[$name]:'';
		$data = in($data);
		return $data;

	}

	protected function istget($name){


		$data = (isset($_GET[$name]) and (!empty($_GET[$name])))?$_GET[$name]:'';
		$data = in($data);
		return $data;
	...
	//2016-11-22
	public function getParentInfo($table,$value,$conditionString=''){



		$info = model($table)->find(" id='$value' $conditionString ");
