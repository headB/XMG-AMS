<?php
class consumablesController extends commonController{
    ...
    if(empty($t)){$req['returnUrl'] = "admin/consumables/index";}
    ...
    $contentArray['subType'] = optionValues(model('assetType')->select("tid='11'"),'typename','--请选择--');
    ...
    $searchInfo = model('consumables')->queryDetail($sql);

    $map1['module']['name'] = '易耗品';
    ...
    public function issueIndex(){
        ...
        $req['returnUrl'] = $t = in($_GET['returnUrl']);
        if(empty($t)){$req['returnUrl'] = "admin/consumables/issueIndex";}
        ...
        $searchInfo = model('consumablesIssue')->queryDetail($sql);
    ...
    public function add(){
        ...
        if($this->ispost()){
            ...
            $info = model('consumables')->insert($data);

            if($info){
                $this->success("数据添加成功！",url('consumables/add'));
            }else{
                $this->error("数据添加失败！",url('consumables/add'));
            }
        ...
        $req['returnUrl'] = url('consumables/add');
    ...
    $data['remark'] = in($_POST['remark']);
        ...
        $info = model('consumables')->update($in,$data);
        ...
        $searchInfo = model('consumables')->queryDetail(" where  a.id='$id'");
        $req = $searchInfo[0];
    ...
    public function issue(){
        ...
        if(empty($returnUrl)){$returnUrl = "r=admin/consumables/index";}
        $url = $_SERVER['SCRIPT_NAME'] . "?" . $returnUrl;
        ...
        if($this->ispost()) {
            ...
            $actualNumber = model('consumables')->find($in);
           ...
           $sofarToal = (int)$actualNumber['actualTotal'] - (int)$data['total'];
            ...
            $info = model('consumables')->update($in,$update);

            if($info){

                $info1 =  model('consumablesIssue')->insert($data);
                if($info1){
                    $this->success("领用登记成功！",$url);
                }else{
                    $infoR = model('consumables')->update($in,$update1);
        ...
        $infoArray = model('consumables')->queryDetail(" where a.id='$t' ");
