...
class dbbackController extends commonController
{
	...
	public function backup()
	{
		...
		self::$ndb->speciallyForBackup($table);
	...
	public function downloadBackupFile(){
		...
		$echoFileName = date('Y-m-d',time())."_Xmg520.sql";


		if(!isset($_GET['request']) or ($_GET['request'])!='xiazaiwenjian'){
			echo 'error';exit;
		}
	...
	header('Content-Disposition: attachment; filename="'.$echoFileName.'"');
