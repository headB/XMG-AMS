...
class indexController extends commonController
{
	...
	public function index()
	{
		...
		$menu=Array(
		   Array('title'=>'管理首页','channels' => Array($root['set'],$root['admin'],$root['dbback'],$root['files'])),//默认显示菜单
				Array('title'=>'电脑租赁','channels' => Array($root['rent'])),
				Array('title'=>'固定资产','channels' => Array($root['asset'])),
				Array('title'=>'易耗品','channels' => Array($root['consumables'])),
				Array('title'=>'非易耗品','channels' => Array($root['nonconsumables'])),
				Array('title'=>'行政核心设置','channels' => Array($root['coreset'])),
				Array('title'=>'总评价信息','channels' => Array($root['hronly'])),
		);
	...
	public function wifiUse(){

		$this->display();

	}

	public function estimateEnter(){

		$contentArray = model('domain')->select();
	...
	//这里备注一下先，这里这个content的作用是可以在某些特殊条件之后的添加某些特殊条件。
	$content = '';
	...
	switch($query){
		...
		case "subjectId":
			...
			$selectLabel = 'subjectTeacherName';
			$condition['tid'] = $field;
			break;
	...
	$selectValue = !empty($selectValue)?$selectValue:'id';
	if(!isset($selectLabel) or empty($selectValue)){return "";}


	if(empty($tableName)){return '';}

	$info = model($tableName)->select($condition);

	if(empty($info) or !is_array($info)){return '';}


	foreach($info as $value){
		$value1 = $value[$selectValue];
