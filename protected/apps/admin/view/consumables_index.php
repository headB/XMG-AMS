...
$content[] = "<button  id='reset' class=\"btn btn-danger\" >重置搜索条件</button>";
...
?>
    ...
    if(isset($searchInfo) and is_array($searchInfo)){
        ...
        foreach($searchInfo as $value){
            ...
            if($map1['module']['name']=="易耗品"){
                $urlEdit = url('consumables/edit')."&id=".$value['id']."&returnUrl=".$returnUrl;
                $urlIssue = url('consumables/issue')."&id=".$value['id']."&returnUrl=".$returnUrl;
            }else{

                $urlEdit = url('nonConsumables/edit')."&id=".$value['id']."&returnUrl=".$returnUrl;
                $urlIssue = url('nonConsumables/issue')."&id=".$value['id']."&returnUrl=".$returnUrl;
            ...
            $content[] = $value['subType'];
            $content[] = $value['measure'];
            $content[] = $value['addTotal'];
            $content[] = (int)$value['addTotal'] - (int)$value['actualTotal'];
            $content[] = $value['actualTotal'];
            $content[] = $value['price'];
            $content[] = $value['totalPrice'];
            $content[] = $value['location'];
            $content[] = "<button class='btn btn-primary'  onclick='window.location.href=\"$urlIssue\"' >领用</button>";
            $content[] = "<button class='btn btn-warning'  onclick='window.location.href=\"$urlEdit\"' >编辑/查看</button>";
